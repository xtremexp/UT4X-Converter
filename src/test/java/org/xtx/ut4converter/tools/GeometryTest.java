...
public class GeometryTest {
    ...
    @Test
    public void testGetRotator(){

        // testing parser
        final Rotator r1 = T3DUtils.parseRotator("BaseRot=(Pitch=32768,Yaw=-16384,Roll=65536)");
        Assert.assertEquals(32768, r1.getPitch(), 0.01d);
        Assert.assertEquals(-16384, r1.getYaw(), 0.01d);
        Assert.assertEquals(65536, r1.getRoll(), 0.01d);
        // test default space is UE123 space (not UE4 space)
        Assert.assertFalse(r1.isUE4Space());

        // test switching from UE123 space to UE4 space (meaning changing range and rotator value)
        r1.switchSpace(true);
        Assert.assertEquals(180, r1.getPitch(), 0.01d);
        Assert.assertEquals(-90, r1.getYaw(), 0.01d);
        Assert.assertEquals(360, r1.getRoll(), 0.01d);
        Assert.assertTrue(r1.isUE4Space());

        final Rotator r2 = T3DUtils.parseRotator("BaseRot=(Yaw=-16384,Roll=65536)");
        Assert.assertEquals(0, r2.getPitch(), 0.01d);
        Assert.assertEquals(-16384, r2.getYaw(), 0.01d);
        Assert.assertEquals(65536, r2.getRoll(), 0.01d);

        final Rotator r3 = T3DUtils.parseRotator("BaseRot=(Pitch=32768,Yaw=-16384)");
        Assert.assertEquals(32768, r3.getPitch(), 0.01d);
        Assert.assertEquals(-16384, r3.getYaw(), 0.01d);
        Assert.assertEquals(0, r3.getRoll(), 0.01d);

        final Rotator r4 = T3DUtils.parseRotator("BaseRot=(Pitch=32768,Roll=65536)");
        Assert.assertEquals(32768, r4.getPitch(), 0.01d);
        Assert.assertEquals(0, r4.getYaw(), 0.01d);
        Assert.assertEquals(65536, r4.getRoll(), 0.01d);
