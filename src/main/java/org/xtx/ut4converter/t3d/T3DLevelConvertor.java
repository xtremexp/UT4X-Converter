...
public class T3DLevelConvertor extends Task<Object> {
	...
	private T3DBrush findBrushByName(String actorname) {
		for (T3DActor uta : convertedActors) {
			if (uta instanceof T3DBrush && uta.getName() != null && uta.getName().equals(actorname)) {
				return (T3DBrush) uta;
			}
		}
	...
	private boolean isBeginActor(String line) {
		...
		if (mapConverter.isFrom(UTGames.UnrealEngine.UE1, UTGames.UnrealEngine.UE2)) {
			return line.contains("Begin Actor");
		}

		// Any actor/sub-class begins with "Begin Object"
		else if (mapConverter.isFrom(UTGames.UnrealEngine.UE3)) {

			if (mapConverter.getInMap().getName().endsWith(".t3d")) {
				return line.contains("Begin Actor") || line.startsWith("Begin Terrain ");
			}
			// handling t3d file exported by UCC which produces "Begin Object" lines
	...
	private boolean isEndActor(String line) {
		...
		// Any actor begin with "Begin Object"
			...
			if (mapConverter.getInMap().getName().endsWith(".t3d")) {
				return line.contains("End Actor") || line.endsWith("End Terrain");
