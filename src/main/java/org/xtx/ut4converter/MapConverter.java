	...
	private void findTexturesUsedInStaticMeshes() {
		...
		for (UPackage unrealPackage : mapPackages.values()) {
			...
			for (UPackageRessource ressource : unrealPackage.getRessources()) {
				...
				final File exportedFile = ressource.getExportInfo().getFirstExportedFile();

				if (exportedFile != null && exportedFile.length() > 0) {
					try {
						final File tempPskDir = new File(exportedFile.getParent() + File.separator + "NEWPSK");
						...
						final String fileExt = FilenameUtils.getExtension(exportedFile.getName());
						...
						// static mesh exported by UCC
							...
							// convert .t3d to .obj
							final StaticMesh t3dMesh = new StaticMesh(exportedFile);
