...
import io.github.xtremexp.ut4converter.UTGames;
...
import io.github.xtremexp.ut4converter.ucore.UPackage;
...
public final class UCCExporter extends UTPackageExtractor {
	...
	public static File exportLevelToT3d(MapConverter mapConverter, File unrealMap) throws Exception {
		...
		if (mapConverter.getInputGame().getUeVersion() == UnrealEngine.UE3.version) {
			final File binariesFolder = new File(mapConverter.getInputGame().getPath() + File.separator + mapConverter.getInputGame().getPkgExtractorPath()).getParentFile();
