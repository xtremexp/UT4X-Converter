...
/**
 * <p>
 * The image metadata for a TGA image type. At this time there are no elements
 * in the format (i.e.
...
public class TGAImageMetadata extends IIOMetadata {
	...
	public TGAImageMetadata() {
		super(TGAImageReaderSpi.SUPPORTS_STANDARD_IMAGE_METADATA_FORMAT, TGAImageReaderSpi.NATIVE_IMAGE_METADATA_FORMAT_NAME, TGAImageReaderSpi.NATIVE_IMAGE_METADATA_FORMAT_CLASSNAME,
				TGAImageReaderSpi.EXTRA_IMAGE_METADATA_FORMAT_NAMES, TGAImageReaderSpi.EXTRA_IMAGE_METADATA_FORMAT_CLASSNAMES);
	}

	/**
	 * <p>
	 * Ensure that the specified format name is supported by this metadata. If
	 * the format is not supported {@link java.lang.IllegalArgumentException} is
	 * thrown.
	 * </p>
	 * 
	 * @param formatName
	 *            the name of the metadata format that is to be validated
	 */
	private void checkFormatName(final String formatName) {
		...
		if (!TGAImageReaderSpi.NATIVE_IMAGE_METADATA_FORMAT_NAME.equals(formatName)) {
			throw new IllegalArgumentException("Unknown image metadata format name \"" + formatName + "\"."); // FIXME:
																												// localize
