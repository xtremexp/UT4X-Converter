...
/**
 * <p>
 * The {@link javax.imageio.ImageReader} that exposes the TGA image reading. 8,
 * 15, 16, 24 and 32 bit true color or color mapped (RLE compressed or
 * uncompressed) are supported. Monochrome images are not supported.
 * </p>
 * 
 * <p>
 * Great care should be employed with {@link javax.imageio.ImageReadParam}s.
 * Little to no effort has been made to correctly handle sub-sampling or
 * specified bands.
 * </p>
 * 
 * <p>
 * {@link javax.imageio.ImageIO#setUseCache(boolean)} should be set to
 * <code>false</code> when using this reader. Also,
 * {@link javax.imageio.ImageIO#read(java.io.InputStream)} is the preferred read
 * method if used against a buffered array (for performance reasons).
 * </p>
 * 
...
public class TGAImageReader extends ImageReader {
	/**
	 * <p>
	 * The {@link javax.imageio.stream.ImageInputStream} from which the TGA is
	 * read. This may be <code>null</code> if
	 * {@link javax.imageio.ImageReader#setInput(java.lang.Object)} (or the
	 * other forms of <code>setInput()</code>) has not been called. The stream
	 * will be set litle-endian when it is set.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * The {@link com.realityinteractive.imageio.tga.TGAHeader}. If
	 * <code>null</code> then the header has not been read since
	 * <code>inputStream</code> was last set. This is created lazily.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Store the input if it is an {@link javax.imageio.stream.ImageInputStream}
	 * . Otherwise {@link java.lang.IllegalArgumentException} is thrown. The
	 * stream is set to little-endian byte ordering.
	 * </p>
	 * 
	...
	public void setInput(final Object input, final boolean seekForwardOnly, final boolean ignoreMetadata) {
		...
		// only ImageInputStream are allowed. If other throw
		// IllegalArgumentException
		if (input instanceof ImageInputStream) {
			// set the inputStream
			inputStream = (ImageInputStream) input;

			// put the ImageInputStream into little-endian
			// ("Intel byte ordering")
			// byte ordering
		...
		{
			throw new IllegalArgumentException("Only ImageInputStreams are accepted."); // FIXME:
																						// localize
		}
	}
	...
	/**
	 * <p>
	 * Create and read the {@link com.realityinteractive.imageio.tga.TGAHeader}
	 * only if there is not one already.
	 * </p>
	 * 
	 * @return the <code>TGAHeader</code> (for convenience)
	 * @throws IOException
	 *             if there is an I/O error while reading the header
	 */
	private synchronized TGAHeader getHeader() throws IOException {
		...
		// read
		if (inputStream == null)
			throw new IllegalStateException("There is no ImageInputStream from which the header can be read."); // FIXME:
																												// localize
	...
	/**
	 * <p>
	 * Only a single image can be read by this reader. Validate the specified
	 * image index and if not <code>0</code> then
	 * {@link java.lang.IndexOutOfBoundsException} is thrown.
	 * </p>
	 * 
	 * @param imageIndex
	 *            the index of the image to validate
	 * @throws IndexOutOfBoundsException
	 *             if the <code>imageIndex</code> is not <code>0</code>
	 */
	private void checkImageIndex(final int imageIndex) {
		// if the imageIndex is not 0 then throw an exception
		if (imageIndex != 0)
			throw new IndexOutOfBoundsException("Image index out of bounds (" + imageIndex + " != 0)."); // FIXME:
																											// localize
	...
	public Iterator/* <ImageTypeSpecifier> */getImageTypes(final int imageIndex) throws IOException {
		...
		default:
			throw new IllegalArgumentException("The image type is not known."); // FIXME:
																				// localize
		}
	...
	public BufferedImage read(final int imageIndex, final ImageReadParam param) throws IOException {
		...
		final int[] intData = ((DataBufferInt) raster.getDataBuffer()).getData(); // CHECK:
																					// is
																					// this
																					// valid
																					// /
		...
		boolean readPixel = true; // if true then a raw pixel is read. Used by
									// the RLE.
		boolean isRaw = false; // if true then the next pixels should be read.
								// Used by the RLE.
		...
		for (int y = 0; y < height; y++) {
			...
			// TODO: this doesn't take into account the destination size or
			// bands
			index *= width;
			...
			for (int x = 0; x < width; x++) {
				// if the image is compressed (run length encoded) then
				// determine
				...
				if (header.isCompressed()) {
					...
					} else /* non-positive run length */
					{
						...
						isRaw = ((runLength & 0x80) == 0); // bit 7 == 0 -> raw;
															// bit 7 == 1 ->
															// runlength
	...
	/**
	 * <p>
	 * Reads and returns an array of color mapped values. If the image does not
	 * contain a color map <code>null</code> will be returned
	 * </p>
	 * 
	 * @param header
	 *            the <code>TGAHeader</code> for the image
	 * @return the array of <code>int</code> color map values or
	 *         <code>null</code> if the image does not contain a color map
	 * @throws IOException
	 *             if there is an I/O error while reading the color map
	 */
