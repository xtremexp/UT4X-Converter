...
import static net.nikr.dds.LoadFileOperation.getBufferedImage;

public class ProfileRunner {

	public ProfileRunner(String[] args) {
		List<String> argsList = Arrays.asList(args);
		if (argsList.contains("-24bit"))
			x100("src" + File.separator + "test" + File.separator + "resources" + File.separator + "net"
					+ File.separator + "nikr" + File.separator + "dds" + File.separator + "gimp" + File.separator
					+ "rgb8.dds");
	...
	private void x100(String filename) {
		...
		for (int i = 0; i < 100; i++) {
			BufferedImage image = getBufferedImage(file, 0);
			for (int imageIndex = 1; image != null; imageIndex++) {
				image = getBufferedImage(file, imageIndex);
			}
	...
	private void all() {
		...
		files.addAll(getDirFiles("src" + File.separator + "test" + File.separator + "resources" + File.separator + "net"
				+ File.separator + "nikr" + File.separator + "dds" + File.separator));
		files.addAll(getDirFiles("src" + File.separator + "test" + File.separator + "resources" + File.separator + "net"
				+ File.separator + "nikr" + File.separator + "dds" + File.separator + "gimp" + File.separator));
		for (File file : files) {
